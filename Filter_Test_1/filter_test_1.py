...
if __name__ == "__main__":
    ...
    for index, row in data.iterrows():
        ...
        print("="*50)
        print(f"Analyzing row {index + 1} of {data.shape[0]}")  # index + 1 because the index starts at 0

        fasta_creator(row.iloc[5], index, "./mySequence.fasta")  # Create the tmp fasta file
        blastn_data = blastn_blaster("./mySequence.fasta", dict_path, 0, 1.0E-09)  # Run the blastn tool
    ...
    data.to_csv("./filtered_data.csv", index=False, header=False)  # Save the data
